...
class TurnstileWebApp:
    ...
    async def _initialize_browser(self) -> None:
        ...
        if not PLAYWRIGHT_AVAILABLE and not CAMOUFOX_AVAILABLE:
            logger.error("No browser automation library available. Please install patchright or camoufox.")
            raise RuntimeError("Browser automation not available")

        if self.browser_type in ['chromium', 'chrome', 'msedge']:
            if not PLAYWRIGHT_AVAILABLE:
                logger.error("Playwright not available for browser type: " + self.browser_type)
                raise RuntimeError("Playwright not available")
            playwright = await async_playwright().start()
        elif self.browser_type == "camoufox":
            if not CAMOUFOX_AVAILABLE:
                logger.error("Camoufox not available")
        ...
        try:
            if self.debug:
                ...
                await self.ws_manager.broadcast("task_progress", {
                    "taskId": task_id, 
                    "status": f"Browser {index}: Starting solve process"
                })
            ...
            if self.debug:
                await self.ws_manager.broadcast("task_progress", {
                    "taskId": task_id, 
                    "status": f"Browser {index}: Setting up Turnstile widget"
                })
            ...
            for attempt in range(10):
                try:
                    await self.ws_manager.broadcast("task_progress", {
                        "taskId": task_id, 
                        "status": f"Browser {index}: Solving attempt {attempt + 1}/10"
                    })
                    ...
                    if turnstile_check == "":
                        ...
                        logger.success(f"Browser {index}: Successfully solved captcha in {elapsed_time} seconds")
            ...
            if self.results.get(task_id) == "CAPTCHA_NOT_READY":
                ...
                await self.ws_manager.broadcast("task_failed", {
                    "taskId": task_id,
                    "error": "Failed to solve captcha after 10 attempts"
                })
...
if __name__ == '__main__':
    ...
    if args.browser_type not in browser_types:
        logger.error(f"Unknown browser type: {COLORS.get('RED')}{args.browser_type}{COLORS.get('RESET')} Available: {browser_types}")
    elif args.headless is True and args.useragent is None and "camoufox" not in args.browser_type:
        logger.error(f"You must specify a {COLORS.get('YELLOW')}User-Agent{COLORS.get('RESET')} for headless mode or use {COLORS.get('GREEN')}camoufox{COLORS.get('RESET')}")
    else:
